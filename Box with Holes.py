...
def generate_shape():
    basic_shape = BRepPrimAPI_MakeBox(1, 1, 1).Shape()
    return basic_shape


def make_holes_1(base):
    feature_diameter = 0.5
    feature_origin = gp_Ax1(gp_Pnt(0.5, 0.5, 0), gp_Dir(0, 0, 1))
...
def make_holes_2(base):
    feature_diameter = 0.5
    feature_origin = gp_Ax1(gp_Pnt(0.5, 0, 0.5), gp_Dir(0, 1, 0))
    feature_maker = BRepFeat_MakeCylindricalHole()
    feature_maker.Init(base, feature_origin)
    feature_maker.Build()
    feature_maker.Perform(feature_diameter / 2.0)
...
def make_holes_3(base):
    feature_diameter = 0.5
    feature_origin = gp_Ax1(gp_Pnt(0, 0.5, 0.5), gp_Dir(1, 0, 0))
    feature_maker = BRepFeat_MakeCylindricalHole()
    feature_maker.Init(base, feature_origin)
    feature_maker.Build()
    feature_maker.Perform(feature_diameter / 2.0)
